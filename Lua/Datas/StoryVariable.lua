local ReadOnly = (DataMgr or {}).ReadOnly or function(n, x)
  return x
end
return ReadOnly("StoryVariable", {
  BH_Thea_Online = {
    IsGlobal = true,
    VarName = "BH_Thea_Online"
  },
  BoxSide = {IsGlobal = true, VarName = "BoxSide"},
  DaDunStage = {IsGlobal = true, VarName = "DaDunStage"},
  DaFengBaoXiang200303 = {
    IsGlobal = true,
    VarName = "DaFengBaoXiang200303"
  },
  East01Batt01Phase = {
    IsGlobal = true,
    VarName = "East01Batt01Phase"
  },
  East01Batt02Phase = {
    IsGlobal = true,
    VarName = "East01Batt02Phase"
  },
  East01Batt03Phase = {
    IsGlobal = true,
    VarName = "East01Batt03Phase"
  },
  East01Nai01Phase = {
    IsGlobal = true,
    VarName = "East01Nai01Phase"
  },
  East01Nai02Phase = {
    IsGlobal = true,
    VarName = "East01Nai02Phase"
  },
  East01Nai03Phase = {
    IsGlobal = true,
    VarName = "East01Nai03Phase"
  },
  EastGodPre = {IsGlobal = true, VarName = "EastGodPre"},
  Entrance110101 = {
    IsGlobal = true,
    VarName = "Entrance110101"
  },
  FaneSide = {IsGlobal = true, VarName = "FaneSide"},
  FeinaDay01Back = {
    IsGlobal = true,
    VarName = "FeinaDay01Back"
  },
  FeinaDay01Catch01 = {
    IsGlobal = true,
    VarName = "FeinaDay01Catch01"
  },
  FeinaDay01Catch02 = {
    IsGlobal = true,
    VarName = "FeinaDay01Catch02"
  },
  FeinaDay01Catch03 = {
    IsGlobal = true,
    VarName = "FeinaDay01Catch03"
  },
  FeinaDay01Home = {
    IsGlobal = true,
    VarName = "FeinaDay01Home"
  },
  FeinaDay01Sleep = {
    IsGlobal = true,
    VarName = "FeinaDay01Sleep"
  },
  FeinaDay01UI = {
    IsGlobal = true,
    VarName = "FeinaDay01UI"
  },
  FeinaDay01Walk = {
    IsGlobal = true,
    VarName = "FeinaDay01Walk"
  },
  FeinaDay02Back01 = {
    IsGlobal = true,
    VarName = "FeinaDay02Back01"
  },
  FeinaDay02Back02 = {
    IsGlobal = true,
    VarName = "FeinaDay02Back02"
  },
  FeinaDay02Home = {
    IsGlobal = true,
    VarName = "FeinaDay02Home"
  },
  FeinaDay02Survey01 = {
    IsGlobal = true,
    VarName = "FeinaDay02Survey01"
  },
  FeinaDay02Survey02 = {
    IsGlobal = true,
    VarName = "FeinaDay02Survey02"
  },
  FeinaDay02UI = {
    IsGlobal = true,
    VarName = "FeinaDay02UI"
  },
  FeinaDay02Walk = {
    IsGlobal = true,
    VarName = "FeinaDay02Walk"
  },
  FeinaDay03Catch = {
    IsGlobal = true,
    VarName = "FeinaDay03Catch"
  },
  FeinaDay03Home = {
    IsGlobal = true,
    VarName = "FeinaDay03Home"
  },
  FeinaDay03HomeBook = {
    IsGlobal = true,
    VarName = "FeinaDay03HomeBook"
  },
  FeinaDay03UI = {
    IsGlobal = true,
    VarName = "FeinaDay03UI"
  },
  FeinaDay03Walk = {
    IsGlobal = true,
    VarName = "FeinaDay03Walk"
  },
  FeinaDay04BackCity01 = {
    IsGlobal = true,
    VarName = "FeinaDay04BackCity01"
  },
  FeinaDay04BackCity02 = {
    IsGlobal = true,
    VarName = "FeinaDay04BackCity02"
  },
  FeinaDay04Fight = {
    IsGlobal = true,
    VarName = "FeinaDay04Fight"
  },
  FeinaDay04Find01 = {
    IsGlobal = true,
    VarName = "FeinaDay04Find01"
  },
  FeinaDay04Find02 = {
    IsGlobal = true,
    VarName = "FeinaDay04Find02"
  },
  FeinaDay04Home = {
    IsGlobal = true,
    VarName = "FeinaDay04Home"
  },
  FeinaDay04HomeCry = {
    IsGlobal = true,
    VarName = "FeinaDay04HomeCry"
  },
  FeinaDay04HomeParty = {
    IsGlobal = true,
    VarName = "FeinaDay04HomeParty"
  },
  FeinaDay04StoryEnd = {
    IsGlobal = true,
    VarName = "FeinaDay04StoryEnd"
  },
  FeinaDay04Thief = {
    IsGlobal = true,
    VarName = "FeinaDay04Thief"
  },
  FeinaDay04UI = {
    IsGlobal = true,
    VarName = "FeinaDay04UI"
  },
  FeinaDay04Walk01 = {
    IsGlobal = true,
    VarName = "FeinaDay04Walk01"
  },
  FeinaDay04Walk02 = {
    IsGlobal = true,
    VarName = "FeinaDay04Walk02"
  },
  FirstGotoIceCity12000103 = {
    QuestChainId = 120001,
    VarName = "FirstGotoIceCity12000103"
  },
  FollowEnlike100206 = {
    IsGlobal = true,
    VarName = "FollowEnlike100206"
  },
  ForgeDanyao = {
    IsGlobal = true,
    VarName = "ForgeDanyao"
  },
  ForgePenhuo = {
    IsGlobal = true,
    VarName = "ForgePenhuo"
  },
  HaierYaoyue1 = {
    IsGlobal = true,
    VarName = "HaierYaoyue1"
  },
  Hesite500080 = {
    IsGlobal = true,
    VarName = "Hesite500080"
  },
  Huozai04Side = {
    IsGlobal = true,
    VarName = "Huozai04Side"
  },
  IQ1002 = {IsGlobal = true, VarName = "IQ1002"},
  IQ1003 = {IsGlobal = true, VarName = "IQ1003"},
  IQ100303 = {IsGlobal = true, VarName = "IQ100303"},
  IQ100304 = {IsGlobal = true, VarName = "IQ100304"},
  IQ10030498 = {IsGlobal = true, VarName = "IQ10030498"},
  IQ100306 = {IsGlobal = true, VarName = "IQ100306"},
  IQ10030601 = {IsGlobal = true, VarName = "IQ10030601"},
  IQ110103 = {IsGlobal = true, VarName = "IQ110103"},
  IQ110107 = {IsGlobal = true, VarName = "IQ110107"},
  JiShuvar1 = {IsGlobal = true, VarName = "JiShuvar1"},
  JiShuvar2 = {IsGlobal = true, VarName = "JiShuvar2"},
  JiShuvar3 = {IsGlobal = true, VarName = "JiShuvar3"},
  KajiaTalk = {IsGlobal = true, VarName = "KajiaTalk"},
  KajiaTalkEnd110105 = {
    IsGlobal = true,
    VarName = "KajiaTalkEnd110105"
  },
  LarryChoice = {
    IsGlobal = true,
    VarName = "LarryChoice"
  },
  LarrySide = {IsGlobal = true, VarName = "LarrySide"},
  LarsSide = {IsGlobal = true, VarName = "LarsSide"},
  LinshiNPC110101 = {
    IsGlobal = true,
    VarName = "LinshiNPC110101"
  },
  Main10030510 = {
    IsGlobal = true,
    VarName = "Main10030510"
  },
  NPC110108 = {IsGlobal = true, VarName = "NPC110108"},
  NengYuanNPC110105 = {
    IsGlobal = true,
    VarName = "NengYuanNPC110105"
  },
  PaotaiStage110105 = {
    IsGlobal = true,
    VarName = "PaotaiStage110105"
  },
  Pet01Battle01 = {
    IsGlobal = true,
    VarName = "Pet01Battle01"
  },
  Pet01Battle02 = {
    IsGlobal = true,
    VarName = "Pet01Battle02"
  },
  PhotoTalk110105 = {
    IsGlobal = true,
    VarName = "PhotoTalk110105"
  },
  Prologue_optimization = {
    IsGlobal = true,
    VarName = "Prologue_optimization"
  },
  SaiqiWalk100206 = {
    IsGlobal = true,
    VarName = "SaiqiWalk100206"
  },
  ShenPan1st = {IsGlobal = true, VarName = "ShenPan1st"},
  ShenPan2nd = {IsGlobal = true, VarName = "ShenPan2nd"},
  ShenWenNPC110105 = {
    IsGlobal = true,
    VarName = "ShenWenNPC110105"
  },
  ShenxunFail110109 = {
    IsGlobal = true,
    VarName = "ShenxunFail110109"
  },
  ShenxunStage110109 = {
    IsGlobal = true,
    VarName = "ShenxunStage110109"
  },
  TeammateEffects = {
    IsGlobal = true,
    OnChangedFunctionName = "OnChanged_TeammateEffects",
    VarName = "TeammateEffects"
  },
  TestVar2 = {
    IsGlobal = true,
    OnChangedFunctionName = "OnChanged_TestVar2",
    VarName = "TestVar2"
  },
  TestVar3 = {QuestChainId = 100101, VarName = "TestVar3"},
  TestVar4 = {
    IsGlobal = true,
    QuestChainId = 100101,
    VarName = "TestVar4"
  },
  TestVar5 = {QuestChainId = 100102, VarName = "TestVar5"},
  TestVar6 = {
    IsGlobal = true,
    QuestChainId = 100102,
    VarName = "TestVar6"
  },
  Weike110103 = {
    IsGlobal = true,
    VarName = "Weike110103"
  },
  XiAoStage = {IsGlobal = true, VarName = "XiAoStage"},
  XimengNPC110105 = {
    IsGlobal = true,
    VarName = "XimengNPC110105"
  },
  icl100303 = {IsGlobal = true, VarName = "icl100303"},
  icl100303bf = {
    IsGlobal = true,
    VarName = "icl100303bf"
  },
  sew100303 = {IsGlobal = true, VarName = "sew100303"},
  thea100306 = {IsGlobal = true, VarName = "thea100306"}
})
